...
object pepon{
    ...
    method come(alimento, cantidad){
    	...
    	if(alimento == "alpiste"){
    		energia = energia + (2 * cantidad)
    		 ...
    		 if(alimento == "mondongo"){
    			energia = energia + (50 * cantidad)
    		}
    		else{
    			if(alimento == "alcaucil"){
    			energia = energia + (10 * cantidad)
    			}
    			else{
    				if(alimento == "sorgo"){
    				energia = energia + (3.5 * cantidad)
    				}
    				else{
    					//Probar darle de comer un BigMac a Pepita. 
    					//Va a dar un error ¿por qué? Arreglar la implementación de bigMac para 
    					...
    					energia = energia + (0.5 * cantidad) //no le va a dar tanta energia
    				}
    			}
    		}
    	}
	...
	//Vuela dependiendo la energia ej1
    method vuela(kilometros){
        energia = energia - 1 - (0.5 * kilometros)
    }
...
object pipa{
    ...
    var contarVecesComida = 0
	
	//Aumento de energia ej1
    method come(alimento, cantidad){
    	
    	contarVecesComida = contarVecesComida + 1
    ...
    method haceLoQueQuieras(){
    	//vacio, averiguar pass, continue o similar
    }
